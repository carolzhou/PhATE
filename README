DO NOT USE THIS CODE. PhATE is now retired, and has been replaced with multiPhATE, at https://github.com/carolzhou/multiPhATE.git.  Thank you.

/Code1/
Note: This is the "original" version of the PhATE pipeline that was submitted for review at Bioinformatics. This Code1 version is set in stone--do not modify. Continuing development is being done in /Code2/, as the single-genome pipeline. The code in /Code3/ modifies /Code2/ to run under a wrapper that controls processing of multiple contigs.
...
$ python phate_runPipeline.py myPhate.config  
...
Phate output is written to a user-specified output directory (specified in your myPhate.config file). In this way, PhATE may be set up to run in parallel on any number of nodes of a compute cluster without fear of clashes in writing results. Implementing a code in parallel is system dependent.
...
The current version of PhATE processes a single phage sequence contig, so run each contig separately through PhATE. In a future release, multi-contig genome sequence files will be processed and combined into a single output directory. 
